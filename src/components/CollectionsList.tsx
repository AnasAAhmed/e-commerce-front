               ...
               { Array.from({ length:import.meta.env.VITE_COLLECTION_LIST_COUNT }, (_,i) => (
                ...
                <ul className="flex flex-wrap justify-center mx-8 mt-3">
                    {data?.collections?.map((collection, i) => (
                        <Link key={i} className="mx-2 mt-2 bg-gray-300 text-gray-600 rounded-md py-1 px-2 font-semibold hover:bg-gray-200" to={`collections/${collection}`}>{collection.toUpperCase()}</Link>
                    ))}
                </ul>
            )}
        </div>
